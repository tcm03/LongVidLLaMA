...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        # video_root varies depending on the environment
    ...
    def _get_length(self):
        """Calculates the number of samples in the csv file."""
        # with open(self.data_path, "r") as file:
        #     for i, _ in enumerate(file):
        #         pass
        # return i + 1  # pyre-fixme
        return len(self.list_data_frame)
    ...
    def __getitem__(self, i: int) -> Dict[str, torch.Tensor]:
        ...
        if os.path.exists(video_path):
            try:
                ...
                print("fail to load video: ", video_path, flush=True)
                return self.__getitem__(0)
        else:
            print("Not exist: ", video_path, flush=True)
            return self.__getitem__(0)
...
def train() -> None:
    ...
    if model_args.vision_tower_aux_list is not None:
        ...
        if "cambrian" in model_args.input_model_filename.lower() or model_args.input_model_filename.lower() == "longvu_qwen2":
