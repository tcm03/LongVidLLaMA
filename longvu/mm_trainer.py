...
#
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            debug_tensor("In evaluation_loop(): After prediction_step: logits", logits)
            debug_tensor("In evaluation_loop(): After prediction_step: labels", labels)
            main_input_name = getattr(self.model, "main_input_name", "input_ids")
            tcm_logger.debug(f"In evaluation_loop(): main_input_name: {main_input_name}")
            ...
            tcm_logger.debug(f"In evaluation_loop(): type(inputs_decode): {type(inputs_decode)}")
            ...
            if logits is not None:
                ...
                if not self.args.batch_eval_metrics or description == "Prediction":
                    ...
                    tcm_logger.debug(f"In evaluation_loop(): len(all_preds): {len(all_preds)}")
                    all_preds.add(logits)
            if labels is not None:
                labels = self.gather_function((labels))
                if not self.args.batch_eval_metrics or description == "Prediction":
                    debug_tensor("Add to all_preds: labels", labels)
                    tcm_logger.debug(f"In evaluation_loop(): len(all_labels): {len(all_labels)}")
                    all_labels.add(labels)

            self.control = self.callback_handler.on_prediction_step(args, self.state, self.control)

            if self.args.batch_eval_metrics:
                tcm_logger.debug(f"In evaluation_loop(): batch_eval_metrics={self.args.batch_eval_metrics}")
            ...
            # Gather all tensors and put them back on the CPU if we have done enough accumulation steps.
                ...
                tcm_logger.debug(f"In evaluation_loop(): eval_accumulation_steps={args.eval_accumulation_steps}")
